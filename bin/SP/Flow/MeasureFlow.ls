                ...
                if self.isCrashMeasure == true and config.system.CCEPMode == true then
                    ...
                    if config.scheduler.measure.mode == MeasureMode.Continous and self.measureType == MeasureType.Sample then
                        while config.scheduler.measure.mode == MeasureMode.Continous and self.measureType == MeasureType.Sample do
                            ...
                            if config.scheduler.collectSample.enable == true and
...
function MeasureFlow:OnStop()
    ...
    if modbusStr == "LS2.0" and config.scheduler.rangeCheck.ZJ.nextStartTime ~= 0 and os.time() > config.scheduler.rangeCheck.ZJ.nextStartTime then
        local onceRangeCheckTime = os.date("%H",config.scheduler.rangeCheck.ZJ.nextStartTime)
        config.scheduler.rangeCheck.timedPoint[onceRangeCheckTime + 1] = false
...
function MeasureFlow:ResultHandle(result)
    ...
    --使用基本量程进行映射
    self.peakArea = self:AreaMap(self.peakArea)
 ...
 * @param[in] area 吸光度
--]]
function MeasureFlow:AreaMap(area)
    local reviseArea = area
    local temp = area / config.measureParam.curveParam[2].curveK
    if temp > 150 then
        reviseArea = 0.000516 * temp * temp + 0.9308 * temp
        reviseArea = reviseArea * config.measureParam.curveParam[2].curveK
        log:debug("使用基本量程进行映射 当前倍数： " .. temp .. "原始面积： " ..  area .. ", 修正面积： " .. reviseArea)
    end
    return reviseArea
end
...
function MeasureFlow:AddParam(param)
    self.measureDateTime = param[1]
    self.currentRange    = param[2]
    self.diluteFactor    = setting.measure.range[param[2]].diluteFactor
end