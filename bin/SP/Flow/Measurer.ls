...
Measurer =
{
    ...
    measureResult =
    {
        ...
        startValue = 0,                 -- 读反应值数组开始值
        endValue = 0,                   -- 读反应值数组结束值
        peakArea = 0,                   -- 结果峰面积数值
    ...
    steps =
    {
        ...
        --5 AddMeasureSample 加待测样
        {
            ...
            execute = function()
                ...
                if Measurer.addParam.sampleVolume > 0 then
                    ...
                    op:ForcingCleanPipe(setting.liquidType.sample, 2, 0.5, 0.5, drainSpeed)
                    --加待测样后清洗水样管
        ...
        --7 exposure 曝气
        {
            ...
            execute = function()
                ...
                log:debug("{Exposure}" .. " time:   " .. config.measureParam.exposureTime )
        ...
        --8 管路更新
        {
            ...
            execute = function()
                ...
                local restTime = config.measureParam.exposureTime - (os.time() - startTime)
                if restTime > 0 then
...
function Measurer:Handle(measureResult)
    ...
    --标记开始值
    measureResult.startValue = dc:GetScanData(measureResult.startIndex - 1)
    --标记结束值
    measureResult.endValue = dc:GetScanData(measureResult.endIndex - 1)
    log:debug("初值：" .. string.format("%0.3f", measureResult.startValue) ..
            ", 末值：" .. string.format("%0.3f", measureResult.endValue))

    --判断末值信号是否饱和
    if measureResult.endValue > setting.measureResult.LedUpperLimit then
        local alarm = Helper.MakeAlarm(setting.alarm.measureLedInvalid, string.format("%0.3f", measureResult.endValue) .. "V")
        AlarmManager.Instance():AddAlarm(alarm)
        error(MeasureLedException:new())    --抛出信号异常
    end
