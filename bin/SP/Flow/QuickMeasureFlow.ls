...
function QuickMeasureFlow:new(o, meaType)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function QuickMeasureFlow:GetRuntime()
    local runtime = 0
...
function QuickMeasureFlow:OnStart()
    --组合流程需要重新加载时间
    ...
    local runStatus = Helper.Status.SetStatus(setting.runStatus.quickMeasure)
...
function QuickMeasureFlow:OnProcess()
    local loopcnt = 3
        ...
        local err,result = pcall(			-- 捕获异常
            function()
                ...
                if self.isCrashMeasure == true and config.system.CCEPMode == true then
                    ...
                    if config.scheduler.quickMeasure.mode == MeasureMode.Continous and self.measureType == MeasureType.Sample then
                        while config.scheduler.quickMeasure.mode == MeasureMode.Continous and self.measureType == MeasureType.Sample do
                            local runStatus = Helper.Status.SetStatus(setting.runStatus.quickMeasure)
                            StatusManager.Instance():SetStatus(runStatus)

                            Measurer:QuickMeasure()
        ...
        if not err then      -- 出现异常
            if type(result) == "userdata" then
                ...
                elseif result:GetType() == "CommandTimeoutException" then          --命令应答超时异常
                    ...
                    log:warn("QuickMeasureFlow:OnProcess =>" .. result:What())
                end
            elseif type(result) == "table" then
                ...
                elseif getmetatable(result) == MeasureDataException then 	 	-- 测量数据异常
                    ExceptionHandler.MakeAlarm(result)
                    self:RetryMeasureCheck()
                elseif getmetatable(result) == MeasureLedException then 	 	-- 测量数据异常
                    ExceptionHandler.MakeAlarm(result)
                    self:RetryMeasureCheck()
                ...
                elseif getmetatable(result) == AirFlowException then        --载气调节异常
                    ...
                    log:warn("QuickMeasureFlow:OnProcess =>" .. result:What())								--其他定义类型异常
                end
            elseif type(result) == "string" then
                log:warn("QuickMeasureFlow:OnProcess =>" .. result)	--C++、Lua系统异常
            end
        end
    end-----重测循环
end
...
function QuickMeasureFlow:OnStop()
    -- 设置测量标志
    ...
    if not self.isFinish then
        status.measure.schedule.autoQuickMeasure.dateTime = self.measureDateTime

        if self.isUserStop then
            self.result = "用户终止"
            log:info("用户终止")
            if self.measureType == MeasureType.Sample and config.scheduler.quickMeasure.mode == MeasureMode.Continous then
                config.scheduler.quickMeasure.mode = MeasureMode.Trigger --连续测量变触发
        ...
        if self.measureType == MeasureType.RangeCheck then
            ...
            if config.measureParam.failAutoRevise == true then
                ...
                if checkError > config.measureParam.checkErrorLimit then
                    if self.autoCheckTimes >= 3 then
                        ...
                        local flow = QuickMeasureFlow:new({}, MeasureType.RangeCheck)
...
--]]
function QuickMeasureFlow:ProformaResultHandle(result)
    --if self.isCheck == true then
    --    Measurer.measureType = MeasureType.Check
    --end
    log:debug("预估测量结果")
    --	print("QuickMeasureFlow:ProformaResultHandle")
    ...
    if self.adjustTime == true then  --排期设置
        if (self.measureType == MeasureType.Sample and config.scheduler.quickMeasure.mode == MeasureMode.Timed) or
...
--]]
function QuickMeasureFlow:ResultHandle(result)
    ...
    if config.scheduler.quickMeasure.mode == MeasureMode.Continous and setting.measureResult.continousModeParam.currentMeasureCnt ~= 0 then
    ...
    elseif config.scheduler.quickMeasure.mode ~= MeasureMode.Continous and setting.measureResult.continousModeParam.currentMeasureCnt ~= 0 then
...
--]]
function QuickMeasureFlow:AbnormalAbsorbanceRevise(absorbance)
...
--]]
function QuickMeasureFlow:CalculateConsistency(area)
    local consistency = 0
    --local peakArea = 0
...
--]]
function QuickMeasureFlow:ReviseAbs(consistency)
    local absorbance = 0
    local curveK = config.measureParam.curveParam[self.currentRange].curveK
    local curveB = config.measureParam.curveParam[self.currentRange].curveB
 ...
 * @brief 检查标线是否为定标
--]]
function QuickMeasureFlow:CheckCurve()
    local ret = true
...
--]]
function QuickMeasureFlow:ConsistencyRevise(consistency,infoPrintf)
    local ret = 0
 ...
 * @param[in] consistency 浓度度
--]]
function QuickMeasureFlow:ConsistencyOffset(consistency)
...
--]]
function QuickMeasureFlow:RangeCheck(result, currange)
 ...
 * @param[in]  currange 当前量程。
--]]
function QuickMeasureFlow:RangeChange(result, currange)
...
--]]
function QuickMeasureFlow:CollectSample()
    local waterCollector = WaterCollector.Instance()
...
--]]
function QuickMeasureFlow:RetryMeasureCheck(isRangeCheck)
...
--]]
function QuickMeasureFlow:SafeResultGenerate()
 ...
 * @brief 流程错误，生成上次测量结果
--]]
function QuickMeasureFlow:FaultGetLastTimeResult()
...
--]]
function QuickMeasureFlow:AdjustMeasureDateTime(meaDataTime)
