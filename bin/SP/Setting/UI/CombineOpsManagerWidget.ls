setting.ui.operation.combineOperator =
{
    ...
    -- row = 10
    {
        ...
        mode = 5,
        createFlow= function(mode, volume)
            local flow = CombineOperateFlow:new({}, setting.liquidType.waste, setting.liquidType.syringeBlank, mode, volume, volume, setting.runAction.syringClean)
            flow.name = setting.ui.operation.combineOperator[10].name
            FlowList.AddFlow(flow)
        end,
        writePrivilege=  RoleType.Administrator,
        readPrivilege = RoleType.Administrator,
