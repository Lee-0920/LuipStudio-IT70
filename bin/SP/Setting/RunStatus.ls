setting.runStatus =
{
    ...
    measureSample =
    {
        ...
        GetTime = function()
            ...
            if config.measureParam.range[config.measureParam.currentRange + 1] + 1 == 1 then
                totalTime = totalTime
            elseif config.measureParam.range[config.measureParam.currentRange + 1] + 1 == 2 then
                totalTime = 650
    ...
    measureZeroCheck =
    {
        ...
        GetTime = function()
            ...
            if config.measureParam.range[config.measureParam.zeroCheckRangeIndex + 1] + 1 == 1 then
                totalTime = totalTime
            elseif config.measureParam.range[config.measureParam.zeroCheckRangeIndex + 1] + 1 == 2 then
                totalTime = 650
            elseif config.measureParam.range[config.measureParam.zeroCheckRangeIndex + 1] + 1 == 3 then
                totalTime = 770
                if true == config.measureParam.wasteWaterEnvironment then   -- 抗污模式
                    totalTime = totalTime + 60
                end
            elseif config.measureParam.range[config.measureParam.zeroCheckRangeIndex + 1] + 1 == 4 then
                totalTime = 1020
            elseif config.measureParam.range[config.measureParam.zeroCheckRangeIndex + 1] + 1 == 5 then
                totalTime = 1300
                if true == config.measureParam.wasteWaterEnvironment then   -- 抗污模式
                    totalTime = totalTime + 60
                end
    ...
    measureRangeCheck =
    {
        ...
        GetTime = function()
            ...
            if config.measureParam.range[config.measureParam.rangeCheckRangeIndex + 1] + 1 == 1 then
                totalTime = totalTime
            elseif config.measureParam.range[config.measureParam.rangeCheckRangeIndex + 1] + 1 == 2 then
                totalTime = 650
            elseif config.measureParam.range[config.measureParam.rangeCheckRangeIndex + 1] + 1 == 3 then
                totalTime = 770
                if true == config.measureParam.wasteWaterEnvironment then   -- 抗污模式
                    totalTime = totalTime + 60
                end
            elseif config.measureParam.range[config.measureParam.rangeCheckRangeIndex + 1] + 1 == 4 then
                totalTime = 1020
            elseif config.measureParam.range[config.measureParam.rangeCheckRangeIndex + 1] + 1 == 5 then
                totalTime = 1300
                if true == config.measureParam.wasteWaterEnvironment then   -- 抗污模式
                    totalTime = totalTime + 60
                end
