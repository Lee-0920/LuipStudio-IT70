...
function Operator:DrainFromWaste()
    ...
    self:Pump(setting.liquidType.none,  0.5, 0.5);
...
function Operator:CheckThermostat(index, temperature, tolerance)
    ...
    if index == setting.temperature.indexStove then
        ...
        if math.abs(currentTemp - setting.temperature.monitor.stoveLastTemperature) < toleranceTemp
            ...
            if setting.temperature.monitor.stoveAlarm == false then
                log:warn("燃烧炉温度异常： 当前温度 " .. currentTemp .. ", 4分钟前温度 " .. setting.temperature.monitor.stoveLastTemperature)
...
function Operator:IsTemperatureAbnormal(index, temperature)
    ...
    if index == setting.temperature.indexStove then
        ...
        local isReach = false
        local isSensorError = false
        --是否达到反应温度
        if math.abs(currentTemp - config.measureParam.reactTemperature) < 20 then
            isReach = true
        end
        --规定时间无法达到预估温度，直接判断传感器异常，停止电炉加热
        if not isReach then
            if setting.temperature.monitor.time > (10 * 60) and currentTemp < 150 then
                isSensorError = true
            elseif setting.temperature.monitor.time > (15 * 60) and currentTemp < 250 then
                isSensorError = true
            elseif setting.temperature.monitor.time > (20 * 60) and currentTemp < 350 then
                isSensorError = true
            elseif setting.temperature.monitor.time > (25 * 60) and currentTemp < 450 then
                isSensorError = true
            elseif setting.temperature.monitor.time > (30 * 60) and currentTemp < 550 then
                isSensorError = true
            elseif setting.temperature.monitor.time > (40 * 60) and currentTemp < config.measureParam.reactTemperature then
                isSensorError = true
            end

            if config.measureParam.reactTemperature < 100 then
                isSensorError = false
            end

            if isSensorError and setting.temperature.monitor.stoveAlarm == false then
                log:warn("燃烧炉加热超时，当前温度 " .. currentTemp .. ", 总加热时长 " .. setting.temperature.monitor.time .. "秒")
                local alarm = Helper.MakeAlarm(setting.alarm.thermostatFault, "燃烧炉温度变化异常")
                AlarmManager.Instance():AddAlarm(alarm)
                setting.temperature.monitor.stoveAlarm = true
                op:StopThermostat()
...
function Operator:SyringUpdateClean(count)
...
function Operator:FillDosingPipe(source, vol, point, wType, fType)
    ...
    -- 设置定量过冲程度
    self:SetMeterOverValue(MeterMode.Accurate, setting.liquid.accurateMeterOverValue * 2)
        ...
        while true do
            local err,result = pcall(function()
                if fillType ==  FillType.FillNormal then
                    ...
                    --op:Meter(MeterMode.Direct, source, maxVol, RollDirection.Suck)

                    self:Pump(source,  0.25, meterSpeed);

                    --精确下压定量到上定量点
                    op:Meter(MeterMode.Direct, setting.liquidType.wasteWater, maxVol, RollDirection.Drain)
                    ...
                    self:Drain(setting.liquidType.master,  0.5, meterSpeed);
