setting.measureScheduler=
{
    ...
    -- 9 周期采样
    {
        ...
        isOpen = function()
            local ret  = false
            if config.interconnection.collectSampleMode == setting.mode.collectSample.period then
                ret = true
            end
            return ret, false
        end,
        ...
        --排期周期
        getInterval = function()
            return config.interconnection.collectSamplePeriod
        end,
        ...
        --下次启动时间
        getNextTime = function()
            return true, status.measure.schedule.autoCollectSample.dateTime + 3600 * config.interconnection.collectSamplePeriod
        end,
        ...
        calculateNextTime = function(startTime, runTime)
            local nextTime = startTime  + 3600 * config.interconnection.collectSamplePeriod
            return true, nextTime
        end,
        ...
        --创建流程
        createFlow = function()
            log:debug("MeasureScheduler createFlow ==> CollectSample")
