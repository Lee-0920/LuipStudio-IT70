...
OOLUA_EXPORT_FUNCTIONS(OpticalMeterInterface, TurnOnLED, TurnOffLED,
                       ...
                       SetRopinessMeterOverValue, SetMeterLedLevel
...
OOLUA_EXPORT_FUNCTIONS_CONST(OpticalMeterInterface, GetPumpFactor, GetMeterPoints,
                             ...
                             GetRopinessMeterOverValue, GetMeterLedLevel,GetMeterEndPointOverCount)
...
OOLUA_EXPORT_FUNCTIONS(TemperatureControlInterface, SetCalibrateFactor, SetThermostatParam,StartThermostat,
                   ...
                   ThermostatFanSetOutput,SetHeaterMaxDutyCycle,SetCurrentThermostatParam, RegisterTemperatureNotice)
OOLUA_EXPORT_FUNCTIONS_CONST(TemperatureControlInterface, GetCalibrateFactor, GetThermostatTemperature, GetThermostatParam,
...
OOLUA_PROXY(,AcquiredOperateResult)
    ...
    OOLUA_ENUMS(
    OOLUA_ENUM(Finished)     ///<采集正常完成。
    OOLUA_ENUM(Failed)      ///< 采集中途出现故障，未能完成。
    OOLUA_ENUM(Stopped)     ///< 采集被停止。
    )
OOLUA_PROXY_END
...
OOLUA_PROXY(,ADCType)
    ...
    OOLUA_ENUMS(
    OOLUA_ENUM(AD7791)      ///< AD7791
    OOLUA_ENUM(ADS1146)     ///< ADS1146
    )
OOLUA_PROXY_END
OOLUA_EXPORT_NO_FUNCTIONS(ADCType)  ///< 导出函数声明（没有函数也需要声明

/**
 * @brief 光学信号采集模块工作模式
 */
OOLUA_PROXY(,ADCWorkMode)
    OOLUA_TAGS(
        Register_class_enums  ///< 枚举需要特别标注，否则枚举值无效
    )
    OOLUA_ENUMS(
    OOLUA_ENUM(OnlyAD7791)      ///< 仅AD7791工作
    OOLUA_ENUM(OnlyADS1146)     ///< 仅ADS1146工作
    OOLUA_ENUM(Both)            ///< AD7791和ADS1146切换工作
    )
OOLUA_PROXY_END
OOLUA_EXPORT_NO_FUNCTIONS(ADCWorkMode)  ///< 导出函数声明（没有函数也需要声明）
...
OOLUA_PROXY(,OpticalAcquireInterface, DeviceInterface)
    ...
    OOLUA_MFUNC_CONST(XonenCtrl)
    OOLUA_MFUNC_CONST(GetADCWorkMode)
    OOLUA_MFUNC_CONST(SetADCWorkMode)
...
OOLUA_EXPORT_FUNCTIONS(OpticalAcquireInterface, SetAcquireADNotifyPeriod, StartAcquirer, StopAcquirer,
                       StartLEDController, SetLEDControllerTarget, SetLEDControllerStableTarget, SetLEDDefaultValue,
...
OOLUA_EXPORT_FUNCTIONS_CONST(OpticalAcquireInterface, GetLEDControllerTartet, GetLEDControllerParam,
                             IsStaticADControlValid, TurnOnLED, TurnOffLED, GetStaticADControlParam, GetLEDDefaultValue,
                             XonenCtrl, GetADCWorkMode, SetADCWorkMode)

/**
 * @brief 驱动控制器。
 * @details
 */
OOLUA_PROXY(,TN67DriveController, BaseController)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
...
OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(TN67DriveController, Init, Uninit, SetBoxFanEnable, GetIPeristalticPump, GetISolenoidValve,
                       GetIOpticalMeter,GetIOpticalAcquire, GetITemperatureControl, Register,ClearAllRemainEvent,ClearThermostatRemainEvent,
                       ClearPumpRemainEvent)
OOLUA_EXPORT_FUNCTIONS_CONST(TN67DriveController, GetReportThermostatTemp, GetReportEnvironmentTemp)

TN67DriveControllerPluginProxy::TN67DriveControllerPluginProxy()
{

}

void TN67DriveControllerPluginProxy::Proxy()
{
    ...
    lua->register_class<ADCType>();
    lua->register_class<ADCWorkMode>();
    ...
    lua->register_class<TN67DriveController>();
}
