 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalAcquireInterface:: StartAcquirer(float acquireTime, Uint8 index)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[5] = {0};
    memcpy(&cmdData[0], &acquireTime, sizeof(acquireTime));
    memcpy(&cmdData[4], &index, sizeof(index));
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalAcquireInterface::SetLEDControllerStableTarget(Uint32 targetAD, Uint32 tolerance, Uint32 timeout)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[12] = {0};
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalAcquireInterface::XonenCtrl(Uint8 code) const
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OAI_CTRL_XONEN, &code, sizeof(code)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);

    if (status != DscpStatus::OK)
    {
        logger->debug("氙灯控制命令应答状态：%d", (int)status);
    }
    return (status == DscpStatus::OK);
}
 ...
 */
ADCWorkMode OpticalAcquireInterface::GetADCWorkMode() const
{
    ADCWorkMode mode;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OAI_GET_ADC_WORK_MODE));
    SyncCaller  syncCaller(m_retries);
    DscpRespPtr resp = syncCaller.Send(cmd);
    if (resp)
    {
        mode = *((ADCWorkMode*)resp->data);
    }
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalAcquireInterface::SetADCWorkMode(ADCWorkMode mode) const
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData = (Uint8)mode;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OAI_SET_ADC_WORK_MODE, &cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);

    if (status != DscpStatus::OK)
    {
        logger->debug("设置ADC工作模式命令应答状态：%d", (int)status);
    }
    return (status == DscpStatus::OK);
}
 ...
 */
AdjustResult OpticalAcquireInterface::ExpectADStableAdjust(long timeout)
{
