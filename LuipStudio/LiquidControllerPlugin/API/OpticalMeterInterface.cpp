...
bool OpticalMeterInterface::TurnOnLED(Uint8 num) const
{
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalMeterInterface::TurnOffLED(Uint8 num) const
{
    Uint16 status = DscpStatus::Error;
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalMeterInterface::IsAutoCloseValve(bool isCloseValve) const
{
    Uint16 status = DscpStatus::Error;
 ...
 * @param index Uint8，目标通道索引号。(0-Meter1 1-Meter2)
 * @param targetAD Uint32，目标AD。
 * @return 状态回应，Uint16，支持的状态有：
 *  - @ref DSCP_OK  操作成功；
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalMeterInterface::StartStaticADControl(Uint8 index, Uint32 targetAD)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[5] = {0};
    ...
    {
        logger->debug("启动静态AD调节命令应答状态：%d", (int)status);
    }
    return (status == DscpStatus::OK);
}
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalMeterInterface::StopStaticADControl()
{
    Uint16 status = DscpStatus::Error;
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalMeterInterface::SetStaticADControlParam(Uint8 index, Uint16 value)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[3] = {0};
    memcpy(cmdData, &index, sizeof(Uint8));
    ...
    if (status != DscpStatus::OK)
    {
        logger->debug("设置静态AD调节参数命令应答状态：%d", (int)status);
    }
    return (status == DscpStatus::OK);
}
 ...
 */
StaticMeterADControlResult OpticalMeterInterface::ExpectStaticADControlResult(long timeout)
{
    StaticMeterADControlResult result = StaticMeterADControlResult::Unfinished;

    DscpEventPtr event = this->Expect(DSCP_EVENT_OMI_STATIC_AD_CONTROL_RESULT, timeout);

    if (event != nullptr)
    {
        result = (StaticMeterADControlResult)(*(event->data));
    }
