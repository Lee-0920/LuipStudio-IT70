...
OOLUA_EXPORT_FUNCTIONS(OpticalAcquireInterface, SetAcquireADNotifyPeriod, StartAcquirer, StopAcquirer,
                       ...
                       SetLEDControllerParam, ExpectADAcquirer,  ExpectLEDOnceAdjust, RegisterAcquireADNotice,
 ...
 * @details
 */
OOLUA_PROXY(,PT63DriveController, BaseController)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
...
OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(PT63DriveController, Init, Uninit, SetBoxFanEnable, GetIPeristalticPump, GetISolenoidValve,
                       GetIOpticalMeter,GetCurrentTemperature,GetIOpticalAcquire, GetITemperatureControl,
                       GetIExtOpticalAcquire, GetIExtTemperatureControl, Register,ClearAllRemainEvent,ClearThermostatRemainEvent,
                       ClearPumpRemainEvent)
OOLUA_EXPORT_FUNCTIONS_CONST(PT63DriveController,GetReportThermostatTemp,GetReportEnvironmentTemp,
                             GetDigestTemperature,
                             GetEnvironmentTemperature)

PT63DriveControllerPluginProxy::PT63DriveControllerPluginProxy()
{

}

void PT63DriveControllerPluginProxy::Proxy()
{
    ...
    lua->register_class<PT63DriveController>();
}
