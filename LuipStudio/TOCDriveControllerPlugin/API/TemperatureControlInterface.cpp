...
TempCalibrateFactor TemperatureControlInterface::GetCalibrateFactorForTOC(Uint8 index) const
{
    TempCalibrateFactor factor = {0, 0, 0};
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool TemperatureControlInterface::SetCalibrateFactorForTOC(Uint8 index, TempCalibrateFactor factor)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[13] = {0};
    memcpy(cmdData, &index, sizeof(Uint8));
    memcpy(cmdData+1, &factor, sizeof(TempCalibrateFactor));
 ...
 * @param level float ,风扇速率，对应高电平占空比。默认为0，风扇关闭。
 */
bool TemperatureControlInterface::BoxFanSetOutputForTOC(Uint8 index, float level)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[5] = {0};
 ...
 * @param level float ,风扇速率，对应高电平占空比。默认为0，风扇关闭。
 */
bool TemperatureControlInterface::DigestionFanSetOutputForTOC(Uint8 index, float level)
{
    Uint16 status = DscpStatus::Error;
 ...
 */
bool TemperatureControlInterface::ReviseThermostatTemp(Uint8 index, ThermostatMode mode, float targetTemp, float toleranceTemp, float timeout)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[14] = {0};
    memcpy(cmdData, &index, sizeof(Uint8));
 ...
 * @brief 控制燃烧炉继电器
 */
bool TemperatureControlInterface::RelayControlForTOC(Uint8 ret)
{
    Uint16 status = DscpStatus::Error;
