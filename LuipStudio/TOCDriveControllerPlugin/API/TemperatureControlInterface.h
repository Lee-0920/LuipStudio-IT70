...
public:
    ...
    bool TurnOffRays();
    //修改恒温参数
    bool ReviseThermostatTemp(Uint8 index, ThermostatMode mode, float targetTemp, float toleranceTemp, float timeout);
    // 查询温度传感器的校准系数。
    TempCalibrateFactor GetCalibrateFactorForTOC(Uint8 index) const;
    // 设置温度传感器的校准系数。
    bool SetCalibrateFactorForTOC(Uint8 index, TempCalibrateFactor factor);
