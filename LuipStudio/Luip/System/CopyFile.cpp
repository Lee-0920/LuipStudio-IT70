...
bool CopyFileAction::ExMemoryDetect(QString& errmsg)
{
    errmsg = "";
#ifdef    _CS_ARM_LINUX
    if (!NFSManager::Instance()->IsNFSEnable())
    {
        FileDetecter fileDetect;    //U盘检测
        if(!fileDetect.UdiskDetect())
        {
            errmsg = "未检测到U盘,数据导出失败.";
            logger->debug("未检测到U盘,数据导出失败.");
            return false;
        }
    }
    else if (!NFSManager::Instance()->IsConnected())
    {
        errmsg = "远程文件服务连接失败,数据导出失败.";
        logger->debug("NFS CAN'T CONNECT,数据导出失败.");
        return false;
    }
#endif
    return true;
...
String CopyFileAction::GetTargetDir()
{
    ...
    luaEngine->GetLuaValue(state, "setting.common.dataCopyDirWin", dataCopyDir);
#else
    String instrumentType;
    luaEngine->GetLuaValue(state, "config.info.instrument.type", instrumentType);
    dataCopyDir = "/mnt/udisk/"+instrumentType+"_";
#endif
