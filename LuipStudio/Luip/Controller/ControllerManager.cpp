...
//    }
    ...
    if((event->code) == DSCP_EVENT_DSI_RESET)
    {
        ...
        this->ThermostatControl();
    }
}

void ControllerManager::ThermostatControl(void)
{
    lua_State * newState = LuaEngine::Instance()->GetThreadState();

    OOLUA::Lua_function call(newState);
    OOLUA::Lua_func_ref thermostatControl;

    try
    {
        OOLUA::get_global(newState, "ThermostatControl", thermostatControl);
        call(thermostatControl);
    }
    catch(OOLUA::Exception e)
    {
        logger->warn("ControllerManager::ThermostatControl() => %s", e.what());
    }
    catch(std::exception e)
    {
        logger->warn("ControllerManager::ThermostatControl() => %s", e.what());
