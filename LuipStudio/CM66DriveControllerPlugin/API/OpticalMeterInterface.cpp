...
/**
 * @brief 查询定量LED光强等级
 * @param num Uint8，目标定量点序号。NUM范围 1 - POINTS_MAX
 * @return level Uint8，光强等级。范围：0-1。
 */
Uint8 OpticalMeterInterface::GetMeterLedLevel(Uint8 num) const
{
    Uint8 value = 0;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OMI_GET_LED_LEVEL, &num, sizeof(num)));
    SyncCaller  syncCaller(m_retries);
    DscpRespPtr resp = syncCaller.Send(cmd);
    if (resp)
    {
        value = *((Uint8*)resp->data);
    }
 ...
 * @brief 设置定量LED光强等级
 * @param num Uint8，目标定量点序号。NUM范围 1 - POINTS_MAX
 * @param level Uint8，设置光强等级。范围：0-1。
  * @return 状态回应，Uint16，支持的状态有：
  *  - @ref DSCP_OK  操作成功；
  *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalMeterInterface::SetMeterLedLevel(Uint8 num, Uint8 level) const
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[2] = {0};
    cmdData[0] = num;
    ...
    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OMI_SET_LED_LEVEL, cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalMeterInterface::StartStaticADControl(Uint8 index, Uint32 targetAD) const
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[5] = {0};
    ...
    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OMI_START_STATIC_AD_CONTROL, cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalMeterInterface::StopStaticADControl() const
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OMI_STOP_STATIC_AD_CONTROL));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
 ...
 */
Uint16 OpticalMeterInterface::GetStaticADControlParam(Uint8 index) const
{
    Uint16 value = 0;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OMI_GET_STATIC_AD_CONTROL_PARAM, &index, sizeof(index)));
    SyncCaller  syncCaller(m_retries);
    DscpRespPtr resp = syncCaller.Send(cmd);
    if (resp)
    {
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalMeterInterface::SetStaticADControlParam(Uint8 index, Uint16 value) const
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[3] = {0};
    memcpy(cmdData, &index, sizeof(Uint8));
    ...
    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OMI_SET_STATIC_AD_CONTROL_PARAM, cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
 ...
 */
bool OpticalMeterInterface::IsStaticADControlValid() const
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OMI_IS_STATIC_AD_CONTROL_VALID));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
 ...
 */
MeterResult OpticalMeterInterface::ExpectMeterResult(long timeout)
{
 ...
 */
StaticADControlResult OpticalMeterInterface::ExpectStaticADControlResult(long timeout)
{
    StaticADControlResult result = StaticADControlResult::Unfinished;

    DscpEventPtr event = this->Expect(DSCP_EVENT_OMI_STATIC_AD_CONTROL_RESULT, timeout);
