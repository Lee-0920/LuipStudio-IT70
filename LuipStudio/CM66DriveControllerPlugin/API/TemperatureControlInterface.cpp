 ...
 * @brief 温度传感器自动校准。
 * @details 因为个体温度传感器有差异，出厂或使用时需要校准。该参数将永久保存。
 *  @param testTemp Float32，测量温度 。
 *  @param realTemp Float32，真实温度 。
 * @return 状态回应，Uint16，支持的状态有：
 *  - @ref DSCP_OK  操作成功；
 *  - @ref DSCP_ERROR 操作失败；
 */
bool TemperatureControlInterface::TemperatureCalibrate(float testTemp, float realTemp)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[8] = {0};
    memcpy(cmdData, &testTemp, sizeof(float));
    memcpy(cmdData+4, &realTemp, sizeof(float));

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_TCI_TEMP_CALIBRATE , cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
