 ...
 */
class LUIP_SHARE NP66OpticalMeterInterface : public DeviceInterface
{
public:
    NP66OpticalMeterInterface(DscpAddress addr);
    // 打开LED灯。
...
//    bool AutoCalibratePump(Uint32 valve);
    ...
    // 查询定量点体积。
    NP66MeterPoints GetMeterPoints() const;
    // 设置定量点体积。
    bool SetMeterPoints(NP66MeterPoints points);
    // 查询定量的工作状态。
    Uint16 GetMeterStatus() const;
    // 开始定量。
    bool StartMeter(NP66RollDirection dir, NP66MeterMode mode, float volume, float limitVolume);
    // 停止定量。
    bool StopMeter();
    // 定量结束自动关闭阀开关
    ...
    // 定量结果事件。
    NP66MeterResult  ExpectMeterResult(int timeout);
    // 自动校准结果事件。
//    NP66MeterResult ExpectAutoCalibrateResult(long timeout);
    // 设定定量速度。
