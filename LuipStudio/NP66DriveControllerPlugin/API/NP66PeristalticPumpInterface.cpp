 ...
 * @param[in] addr 设备地址。
 */
NP66PeristalticPumpInterface::NP66PeristalticPumpInterface(DscpAddress addr)
 ...
 * @return 总泵数目， Uint16。
 */
int NP66PeristalticPumpInterface::GetTotalPumps()
{
    Uint16 pumps = 0;
 ...
 */
float NP66PeristalticPumpInterface::GetPumpFactor(int index)
{
    float factor = 0;
 ...
 * @return 返回状态，操作是否成功。
 */
bool NP66PeristalticPumpInterface::SetPumpFactor(int index, float factor)
{
 ...
 */
NP66MotionParam NP66PeristalticPumpInterface::GetMotionParam(int index)
{
    NP66MotionParam param;
    ...
    if (resp)
    {
        param = *((NP66MotionParam*)resp->data);
    }
...
* @return 返回状态，操作是否成功。
 */
bool NP66PeristalticPumpInterface::SetMotionParam(int index, NP66MotionParam param)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[9];

    cmdData[0] = index;
    memcpy(cmdData+1, &param, sizeof(NP66MotionParam));
 ...
 */
NP66PumpStatus NP66PeristalticPumpInterface::GetPumpStatus(int index)
{
    NP66PumpStatus status = NP66PumpStatus::Idle;
    ...
    if (resp)
    {
        status = (NP66PumpStatus)*resp->data;
    }
 ...
 */
bool NP66PeristalticPumpInterface::StartPump(int index, NP66RollDirection dir, float volume, float seep)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[10] = {0};
 ...
 * @return 返回状态，操作是否成功。
 */
bool NP66PeristalticPumpInterface::StopPump(int index)
{
    Uint16 status = DscpStatus::Error;
 ...
 */
float NP66PeristalticPumpInterface::GetPumpVolume(int index)
{
    float volume = 0;
 ...
 * @param[out] result Uint8，泵操作结果码（ @ref NP66PumpResult ），定义如下：
 *  - @ref Finished 泵操作正常完成
 *  - @ref Failed 泵操作中途出现故障，未能完成
 *  - @ref Stopped 泵操作被停止
 */
NP66PumpResult NP66PeristalticPumpInterface::ExpectPumpResult(long timeout)
{
    NP66PumpResult pumpResult;
